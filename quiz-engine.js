...
const QuizEngine = (() => {
  ...
  async function load(url = "./quiz-data.json") {
    // 本番: quizNewsData.js の QUIZ_NEWS_PACK を優先（file://でも動く）
  ...
  function pickQuestion({ difficulty, mimicType } = {}) {
    ...
    // フォールバック: 種別 → 難易度 → 全体（戦闘が空振りしないように）
    let pool = all;
    if (mimicType) {
      const byType = all.filter((q) => q.mimicType === mimicType);
      if (byType.length) pool = byType;
    }
    if (pool === all && difficulty != null) {
      const byDiff = all.filter((q) => q.difficulty === difficulty);
      if (byDiff.length) pool = byDiff;
    } else if (mimicType && difficulty != null && pool.length === 0) {
      const byDiff = all.filter((q) => q.difficulty === difficulty);
      pool = byDiff.length ? byDiff : all;
    }
